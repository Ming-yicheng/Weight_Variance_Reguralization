    ...
    def weight_prune(
        ...
        for name, module in model.named_modules():
            if ( isinstance(module, nn.Conv2d) ):
                ...
                layer_to_rank[name] = np.zeros_like(module.weight.data.cpu().numpy())
        ...
        for layer_name, (input_coverage, output_coverage) in accumulate_coverage.items():
            ...
            current_module = module_dict[layer_name]
            
            if isinstance(current_module, torch.nn.Conv2d) and current_module.groups == current_module.in_channels:
                # 深度卷积处理
                assert input_dim == output_dim, "DW conv should have same input/output channels"
                for channel_idx in range(input_dim):
                    # 只有对角线关系：输入通道i -> 输出通道i
                    coverage_score = input_coverage[channel_idx] + output_coverage[channel_idx]
                    all_weight_coverage.append((coverage_score, (layer_name, 0, channel_idx)))
                    # 注意：input_idx固定为0，output_idx=channel_idx
            else:
                # 普通卷积处理
        ...
        for (coverage_score, pos) in sorted_coverage:
            ...
            if isinstance(current_module, torch.nn.Conv2d) and current_module.groups == current_module.in_channels:
                # 对于深度卷积，input_idx应该总是0
                # output_idx对应实际的通道索引
                layer_to_rank[layer_name][output_idx, 0] = accumulate_index
            else:
