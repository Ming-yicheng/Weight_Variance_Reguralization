...
def adv_coverage(
    ...
    for idx, (images, label) in enumerate(loader):
        ...
        # labal_npy = np.expand_dims(label.cpu().numpy(), 0)
        # 修改后
        labal_npy = np.expand_dims(label, 0)
        ...
        label_tensor = torch.from_numpy(label).to(pred_clean.device)
        clean_correct = pred_clean.eq(label_tensor)
        # clean_correct = pred_clean.eq(label)
        adv_trial += int(clean_correct.sum().item())
        adv_success += int(pred_adv[clean_correct].eq(label[clean_correct]).sum().detach().item())
        top1_clean += int(pred_clean.eq(label).sum().detach().item())
...
def load_student(ckpt, args, num_classes):
    ...
    parent_dir = "generated_pkls/" + args.dataset   #改
...
if __name__ == '__main__':
    ...
    Retrain, Retrain_converter = load_student(args.Retrain_ckpt, args, test_set.num_classes)
    Renofeation, Renofeation_converter = load_student(args.Renofeation_ckpt, args, test_set.num_classes)
    ReMoS, ReMoS_converter = load_student(args.ReMoS_ckpt, args, test_set.num_classes)
    IPLA, IPLA_converter = load_student(args.IPLA_ckpt, args, test_set.num_classes)
    SeaM, SeaM_converter = load_student(args.SeaM_ckpt, args, test_set.num_classes)
    WVR, WVR_converter = load_student(args.WVR_ckpt, args, test_set.num_classes)
    WVR_outlier, WVR_outlier_converter = load_student(args.WVR_outlier_ckpt, args, test_set.num_classes)
