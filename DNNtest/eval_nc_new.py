...
# from eval_clean import LabelConverter  # --- 已删除: 不再需要 LabelConverter ---
...
# --- 已修改: 函数签名不再接收 converters 和 dataset_converter ---
def adv_coverage(
    model, loader, args, bounds, students,
):
    model.eval()
    low_bound, up_bound = bounds
    ...
    for idx, (images, label) in enumerate(loader):
        ...
        # --- 已修改: 直接获取学生模型的预测，不再使用 converter ---
        student_pred_clean = np.array([
            m(images).max(dim=1)[1].cpu().numpy()
            for m in students
        ])
        
        # --- 已修改: 直接从 tensor 格式的 label 创建 numpy 数组 ---
        labal_npy = np.expand_dims(label.cpu().numpy(), 0)
        ...
        # --- 已修改: 直接获取学生模型的预测，不再使用 converter ---
        student_pred_adv = np.array([
            m(adv_images).max(dim=1)[1].cpu().numpy()
            for m in students
        ])
        
        # --- 已修改: 直接使用 tensor 格式的 label 进行比较 ---
        clean_correct = pred_clean.eq(label)
        
        adv_trial += int(clean_correct.sum().item())
        # --- 已修改: 确保 label[clean_correct] 操作有效 (因为 label 保持为 tensor) ---
        adv_success += int(pred_adv[clean_correct].eq(label[clean_correct]).sum().detach().item())
        top1_clean += int(pred_clean.eq(label).sum().detach().item())
        top1_adv += int(pred_adv.eq(label).sum().detach().item())
...
def get_args():
    ...
    parser.add_argument("--ReMoS_ckpt", type=str, default='')
    parser.add_argument("--IPLA_ckpt", type=str, default='')
    parser.add_argument("--SeaM_ckpt", type=str, default='')
    parser.add_argument("--WVR_ckpt", type=str, default='')
    parser.add_argument("--WVR_outlier_ckpt", type=str, default='')
...
def load_student(ckpt, args, num_classes):
    ...
    if isinstance(checkpoint, dict) and 'state_dict' in checkpoint:
        # 如果是标准格式，就从中取出 state_dict
        state_to_load = checkpoint['state_dict']
    else:
        # 否则，假定 checkpoint 本身就是 state_dict
        state_to_load = checkpoint
    
    #使用处理过的 state_dict 加载权重
    model.load_state_dict(state_to_load, strict=False)
    
    print(f"Loaded checkpoint from {ckpt}")
    model.eval()

    # --- 已删除: 加载 class_names 和创建 LabelConverter 的逻辑 ---
    # parent_dir = "generated_pkls/" + args.dataset
...
if __name__ == '__main__':
    ...
    ###
    # --- 已修改: 调用 load_student 时不再接收 converter ---
    Finetune = load_student(args.Finetune_ckpt, args, test_set.num_classes)
    Retrain = load_student(args.Retrain_ckpt, args, test_set.num_classes)
    Renofeation = load_student(args.Renofeation_ckpt, args, test_set.num_classes)
    ReMoS = load_student(args.ReMoS_ckpt, args, test_set.num_classes)
    IPLA = load_student(args.IPLA_ckpt, args, test_set.num_classes)
    SeaM = load_student(args.SeaM_ckpt, args, test_set.num_classes)
    WVR = load_student(args.WVR_ckpt, args, test_set.num_classes)
    WVR_outlier = load_student(args.WVR_outlier_ckpt, args, test_set.num_classes)

    modes = [
        "Finetune", "Retrain", "Renofeation", "ReMoS", "IPLA", "SeaM", "WVR", "WVR_outlier"
    ]
    students = [
        eval(mode) for mode in modes
    ]
    # --- 已删除: student_converters 列表的创建 ---
    # student_converters = [
    #     eval(f"{mode}_converter") for mode in modes
    # ]

    pretrained_model = eval('{}_dropout'.format(args.network))(pretrained=True).cuda()
    # --- 已修改: 调用 adv_coverage 时不再传递 converters ---
