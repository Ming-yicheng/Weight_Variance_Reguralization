...
def get_args():
    ...
    # Weight prune
    parser.add_argument("--weight_total_ratio", default=0, type=float)
    parser.add_argument("--weight_ratio_per_prune", default=0, type=float)
    parser.add_argument("--weight_init_prune_ratio", default=0, type=float)
    ...
    parser.add_argument('--weights-prune-ratio', type=float, default=0.2, help='权重的剪枝率')
...
if __name__=="__main__":
    ...
    elif args.method == "IPLA":
        finetune_machine = IPLA(
            args,
            model, teacher,
            train_loader, test_loader,
        )
    elif args.method == "Variance_regularization":
        finetune_machine = Variance_regularization(
            args, model, teacher, train_loader, test_loader,
        )
    else:
        raise RuntimeError
